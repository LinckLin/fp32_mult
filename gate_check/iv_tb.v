// iverilog gate-level golden replay: latency-5 streaming check
`timescale 10ps/1ps
module iv_tb;
  ...
  reg [71:0] stim [0:100199];
  reg [79:0] exp  [0:100199];
  integer n = 100200;
  integer i, recv, errs, cycles, watchdog;
  reg [4:0]  efmt;
  reg [63:0] eres;
  reg [15:0] efl;
    ...
    fork
      ...
      begin : check
        while (recv < n) begin
          ...
          cycles = cycles + 1;
          if (cycles > 10*n + 100) begin
            $display("WATCHDOG: stuck at recv=%0d", recv);
            $finish;
          end
        end
        $display("IVERILOG GATE CHECK: %0d vectors, %0d mismatches", recv, errs);
